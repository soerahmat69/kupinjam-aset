...
export const MEditPermintaan = ({data, setcloseStatus }) => {
  const [isPeminjam, setisPeminjam] = useState([]);
  const [formData,setformData] = useState({
    keperluan:data.keperluan,
    waktu_jam:data.waktu_jam,
    waktu_pinjam:data.waktu_pinjam,
    waktu_tanggal:data.waktu_tanggal,
    status:data.status
  })
  useEffect(() => {
    axios
  ...
  const PostData = (e)=>{
    ...
    axios
    .put(`http://localhost:8080/admin/request/edit/${data._id}`,formData).then(()=>{
            ...
            <div className="grid gap-4 mb-4 grid-cols-2">
              ...
              <div className="col-span-2 sm:col-span-1">
                ...
                <input
                  ...
                  type="time"
                  defaultValue={formData && formData.waktu_jam}
              ...
              <div className="col-span-2 sm:col-span-1">
                ...
                <input
                 onChange={(e)=>{setformData({...formData,waktu_tanggal:e.target.value})}}
                  type="date"
                  defaultValue={formData && formData.waktu_tanggal}
                  className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                 required
                />
              </div>
              <div className="col-span-2">
                ...
                <input
                 onChange={(e)=>{setformData({...formData,waktu_pinjam:e.target.value})}}
                  id="description"
                  defaultValue={formData && formData.waktu_pinjam}
                  required
                  type="number"
              ...
              <div className="col-span-2">
                ...
                <textarea
                 onChange={(e)=>{setformData({...formData,keperluan:e.target.value})}}
                  id="description"
                  rows="4"
                  defaultValue={formData && formData.keperluan}
                  className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
