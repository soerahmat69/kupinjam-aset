...
import { setDataExport } from "../../App/Store/ExportData";
...
export const Main = ({ children }) => {
 ...
 const { data } = useSelector((state) => state.exportsData); 
//  DataPrint
  const getData = async (tahun = 0, bulan = 0) => {

    try {
      const response = await axios.get(`http://localhost:8080/admin/dashboard/${tahun}/${bulan}`);
 ...
 const handleDatedata = (id)=>{
  ...
  const handleExportData =   (data) => {
    ...
    // Menyimpan data di state lokal
    localStorage.setItem("exportData", JSON.stringify(dashboardData.DataPrint));
  
    return window.open("/admin/exportpdf", "_blank");
  };
        ...
        <div className="flex gap-[30px] w-full     ">
          <div className="flex-none w-[375px] h-[240px] px-[22px] bg-[#f0f0f0] rounded-[15px] border border-solid border-[#252b4880] shadow-[0px_4px_4px_#00000040]">
            ...
            <ul className=" w-full h-max font-[poppins] ">
                ...
                {dashboardData && dashboardData.totalitas ? (
                  <>
                    <Countdown
                      ...
                      etcValue={"+"}
                      finalValue={dashboardData.totalitas.asset}
                    />
                  </>
                ) : (
                  <>
              ...
              <li className="flex justify-between items-center">
                <h1 className="  top-[90px] left-0   font-medium text-[#252b48] text-[20px] tracking-[0] leading-[normal]">
                  Total Perawatan
                </h1>
                {dashboardData && dashboardData.totalitas ? (
                  <>
                    <Countdown
                      ...
                      etcValue={"+"}
                      finalValue={dashboardData.totalitas.perawatan}
                    />
                  </>
                ) : (
                  <>
