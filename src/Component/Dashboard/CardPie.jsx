...
export const CardPie = ({ titlePie, LabelsData, dataArray, IdVispie }) => {
  ...
  const totalitasClick = () => setMore(!More);
  const hapusVisPie = async () => {
    await axios
      .delete(`http://localhost:8080/admin/visualpie/delete/${IdVispie}`)
      .then((res) => {
  ...
  return (
    <div className="   grow  py-5 flex flex-col  font-[poppins] h-[394px] rounded-[15px] border border-solid border-[#252b4880] shadow-[0px_4px_4px_#00000040]">
      <div className="relative flex-none">
        <div
          onClick={totalitasClick}
          className="cursor-pointer absolute w-[55px] h-[28px] right-4 top-1 rotate-[-179.00deg]  font-semibold text-[#252b48] text-[35px] text-center tracking-[4.38px] leading-[normal] whitespace-nowrap"
        >
          ...
        </div>
        <div
          className={`${
            !More && "hidden"
          }   top-[10px] -right-[40px] z-10 absolute overflow-hidden w-max rounded-md shadow-lg bg-[#F0F0F0] ring-1 ring-[#252B4880] `}
        >
